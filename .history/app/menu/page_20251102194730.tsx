...
import { useOrderStore } from "@/store/orderStore"; // ✅ Añadido
...
export default function MenuPage() {
  ...
  const { addToMesa } = useOrderStore(); // ✅ añadimos pedidos
  const { id: mesaId } = useParams() as { id: string }; // ✅ Identificamos mesa
  const router = useRouter(); // ✅ Para navegar a Cuenta
  ...
  const handleSendOrder = () => {
    ...
    addToMesa(mesaId, items); // ✅ Guardamos en una "cuenta" por mesa
    clearCart(); // ✅ Limpiar carrito
    router.push(`/mesa/${mesaId}/cuenta`); // ✅ Ir a la cuenta
  };
    ...
    <main className="min-h-screen bg-[var(--color-background)] p-4 pb-32 max-w-md mx-auto">
      ...
      <div className="fixed bottom-0 left-0 w-full bg-[var(--color-surface)] border-t border-[var(--color-border)] shadow-lg">
        <div className="max-w-md mx-auto p-4">
          ...
          {/* ✅ Botón para ver la cuenta en cualquier momento */}
          <Button
            fullWidth
            variant="outline"
            className="mt-2"
            onClick={() => router.push(`/mesa/${mesaId}/cuenta`)}
          >
            👀 Ver Cuenta
          </Button>
        </div>
