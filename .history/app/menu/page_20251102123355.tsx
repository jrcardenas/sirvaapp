...
export default function MenuPage() {
  ...
  const [openCategory, setOpenCategory] = useState<string>("Bebidas");
  ...
  const toggleCategory = (categoria: string) => {
    const newState = openCategory === categoria ? "" : categoria;
    setOpenCategory(newState);

    setTimeout(() => {
      const headerEl = document.getElementById(`cat-${categoria}`);
  ...
  return (
    <main className="min-h-screen bg-[var(--color-background)] p-4 pb-32 max-w-md mx-auto">
      ...
      <h1 className="text-3xl font-bold text-center mb-4 text-[var(--color-text-dark)]">Carta</h1>

      {categorias.map((categoria) => (
        <div key={categoria} className="mb-3">
          <button
            id={`cat-${categoria}`}
            className="w-full flex justify-between items-center text-lg font-bold py-3 
                    ...
                    <div key={item.id} className="flex justify-between">
                      ...
                      <div className="flex items-center gap-2">
                        <button
                          ...
                          className={`w-8 h-8 rounded-full text-lg font-bold flex items-center justify-center border
                            ${
                              qty === 0
                        ...
                        <span
                          className={`w-6 text-center font-semibold text-lg
                        ...
                        <button
                          className="w-8 h-8 bg-[var(--color-primary)] text-white rounded-full text-lg font-bold flex items-center justify-center"
                          onClick={() =>
