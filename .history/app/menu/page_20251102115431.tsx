...
export default function MenuPage() {
  const { addItem, changeQty, items, removeItem } = useCartStore();
  ...
  // ✅ SUMA todas las cantidades del producto aunque haya notas
    ...
    <main className="min-h-screen bg-[var(--color-background)] p-6 pb-20">
      ...
      <div className="grid gap-4 max-w-md mx-auto">
        {productos.map((item) => {
          ...
          const handleMinus = () => {
            if (qty <= 1) {
              removeItem(item.id); // ✅ Se elimina si era la última unidad
            } else {
