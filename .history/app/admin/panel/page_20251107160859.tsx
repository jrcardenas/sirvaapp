    ...
    <main className="min-h-screen bg-background p-5 pt-24 max-w-lg mx-auto relative">
      ...
      <div className="space-y-6">
        {mesasOrdenadas.map((mesaId) => {
          ...
          // ✅ Unificamos productos iguales (mismo id + precio)
          const agrupados = Object.values(
            (mesa.items ?? []).reduce<Record<string, GrupoItem>>((acc, it) => {
              ...
              it.entregado ? acc[key].servidos++ : acc[key].pendientes++;
              return acc;
            }, {})
          );
          ...
          const totalCuenta = agrupados.reduce(
            (a, i) => a + i.price * (i.servidos + i.pendientes),
            0
          );
                      ...
                      {productosFiltrados.length > 0 ? (
                        productosFiltrados.map((p) => (
                          <button
                            key={p.id}
                            onClick={() =>
                              addCustomProduct(mesaId, p.name, p.price)
                            }
...
/* ✅ Modal editable de cobro con productos unificados */
function ModalCobroEditable({
  ...
  // ✅ Agrupamos productos servidos (unificados por id+price)
  const agrupados = Object.values(
    (mesa.items ?? [])
