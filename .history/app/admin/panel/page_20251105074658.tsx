...
export default function AdminPanel() {
  ...
  // ✅ Nuevos pedidos en tiempo real
useEffect(() => {
  const ch = new BroadcastChannel("pedidos_channel");
  ch.onmessage = (ev) => {
    // Fuerza refrescar Zustand con los nuevos pedidos
  ...
  return (
    <main className="min-h-screen bg-background p-5 pt-24 max-w-lg mx-auto relative">
      ...
      {/* Banners arriba */}
      {Object.entries(mesas).filter(([, m]) => m.llamada)
        .map(([mesaId]) => (
          <div key={mesaId} className="bg-danger text-white font-bold text-center py-3 rounded-xl mb-3 animate-pulse">
            🛎 Mesa {mesaId} llama al camarero
          </div>
        ))}

      {Object.entries(mesas).filter(([, m]) => m.cuentaSolicitada)
        .map(([mesaId]) => (
          <div key={mesaId} className="bg-blue-600 text-white font-bold text-center py-3 rounded-xl mb-3 animate-pulse">
          ...
          {mesasActivas.map(([mesaId, mesa]) => {
            ...
            return (
              <div key={mesaId} className="p-4 rounded-xl shadow-md border border-border bg-surface">
                ...
                {/* ✅ Atender llamada (si hay llamada) */}
                {mesa.llamada && (
                  <button
                ...
                {/* ✅ Atender Cuenta (solo si la pidió) */}
                {mesa.cuentaSolicitada && (
                  <button
                ...
                {/* ✅ Cobrar mesa cuando ya se atendió la cuenta o nunca la pidió */}
                {!mesa.llamada && (
                  <button
