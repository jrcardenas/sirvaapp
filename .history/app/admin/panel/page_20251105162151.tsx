...
import { MENU_ITEMS } from "@/data/menu.js"; // ✅ IMPORTAMOS MENÚ GLOBAL
...
export default function AdminPanel() {
  ...
  const agregarProductoMenu = (mesaId: string, product: any) => {
                      ...
                      <div className="max-h-36 overflow-y-auto bg-white border rounded-xl shadow-md p-1 text-sm">
                        {productosFiltrados.length > 0 ? (
                          ...
                          <div className="px-3 py-2 text-gray-600">
                            ❌ Sin resultados
                            <br />
                            ✅ Crear: <strong>{search}</strong>
                            <button
                              className="block mt-2 w-full bg-primary text-white py-2 rounded-xl font-bold"
                              onClick={() => {
                                addCustomProduct(mesaId, search, 0);
                                setSearch("");
                                setMesaAdding(null);
                              }}
                            >
