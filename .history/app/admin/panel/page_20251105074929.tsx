...
export default function AdminPanel() {
  ...
  useEffect(() => {
    ...
    if (dingRef.current) dingRef.current.volume = 1.0;
    setSoundLoaded(true);
  }, []);
  ...
  // ✅ Eventos desde notify_channel
  useEffect(() => {
    const notify = new BroadcastChannel("notify_channel");
    notify.onmessage = () => dingRef.current?.play().catch(() => null);
    return () => notify.close();
  }, []);

  // ✅ Eventos desde pedidos_channel (nuevos pedidos)
  useEffect(() => {
