      ...
      {mesasActivas.length === 0 ? (
        ...
        <div className="space-y-6">
          {mesasActivas.map(([mesaId, mesa]) => {
            ...
            // ✅ Agrupar productos pendientes por id
            const pendientesAgrupados = pendientes.reduce((acc, item) => {
              if (!acc[item.id])
                acc[item.id] = { ...item, qty: 0, timestamps: [] };

              acc[item.id].qty += 1;
              acc[item.id].timestamps.push(item.timestamp);
              return acc;
            }, {} as Record<string, { qty: number; name: string; timestamps: number[]; price: number }>);
