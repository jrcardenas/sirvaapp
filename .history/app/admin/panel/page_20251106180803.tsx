...
export default function AdminPanel() {
  ...
  // ✅ Sincronización segura en móvil
  useEffect(() => {
  ...
  const mesasOrdenadas = mesasIds.sort((a, b) => {
    const ma = mesas[a] ?? { items: [] as PedidoItem[], llamada: false, cuentaSolicitada: false };
    const mb = mesas[b] ?? { items: [] as PedidoItem[], llamada: false, cuentaSolicitada: false };
    ...
    if (ma.cuentaSolicitada !== mb.cuentaSolicitada) return ma.cuentaSolicitada ? -1 : 1;
    ...
    <main className="min-h-screen bg-background p-5 pt-24 max-w-lg mx-auto relative">
      ...
      <div className="space-y-6">
        {mesasOrdenadas.map((mesaId) => {
          const mesa = mesas[mesaId] ?? { items: [] as PedidoItem[], llamada: false, cuentaSolicitada: false };
...
function ModalCobro({
  ...
  setMesaParaCobrar: (m: string | null) => void;
}) {
  const { mesas, cobrarMesa } = useOrderStore();
  const mesa = mesas[mesaParaCobrar] ?? { items: [] as PedidoItem[] };

  const itemsAgrupados = Object.values(
    (mesa.items ?? [])
      .filter(i => i.entregado)
      .reduce<Record<string, { name: string; price: number; qty: number }>>(
        (acc, item) => {
          if (!acc[item.id]) {
            acc[item.id] = { name: item.name, price: item.price, qty: 0 };
          }
          acc[item.id].qty++;
          return acc;
        },
  ...
  const total = itemsAgrupados.reduce((a, i) => a + i.price * i.qty, 0);
      ...
      <div className="bg-white rounded-2xl p-6 w-80 text-gray-900 shadow-xl">
        ...
        <button
          ...
          onClick={() => {
            ...
            const notify = new BroadcastChannel("notify_channel");
            notify.postMessage({ mesaId: mesaParaCobrar, mensaje: "mesaCobrada" });
            notify.close();
