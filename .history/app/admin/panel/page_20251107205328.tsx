...
export default function AdminPanel() {
  ...
  useEffect(() => {
    ...
    pedidos.onmessage = (ev) => {
      ...
      if (
        ["nuevoPedido", "pedidoConfirmado", "pedidoEnPreparacion", "pedidoListo", "llamada", "cuenta"].includes(
          data.tipo
        )
      ) {
  ...
  return (
    <main className="min-h-screen bg-background p-5 pt-24 max-w-lg mx-auto relative">
        ...
        {mesasIds.map((mesaId) => {
          ...
          const agrupados = Object.values(
            (mesa.items ?? []).reduce<Record<string, GrupoItem>>((acc, it) => {
              ...
              else if (it.estado === "pendiente" || it.estado === "pendienteCocina")
                acc[it.id].pendientes++;
              else if (it.estado === "enCocina") acc[it.id].enCocina++;
              else if (it.estado === "listo") acc[it.id].listos++;
              return acc;
            }, {})
          );

          const allServed = agrupados.every((i) => i.pendientes + i.enCocina + i.listos === 0);
          ...
          return (
            <div key={mesaId} className="p-4 rounded-2xl border shadow bg-white">
              ...
              {/* 🟡 Pendientes */}
              {agrupados.some((i) => i.pendientes > 0) && (
                <div className="mb-2">
                  <h3 className="text-yellow-700 font-semibold mb-1">🟡 Pendientes</h3>
                  {agrupados
                    .filter((i) => i.pendientes > 0)
                    .map((item) => (
                      <div
                        key={`${item.id}-pendientes`}
              ...
              {/* 🍳 En cocina */}
              {agrupados.some((i) => i.enCocina > 0) && (
                <div className="mb-2">
                  <h3 className="text-orange-700 font-semibold mb-1">🍳 En cocina</h3>
                  {agrupados
                    .filter((i) => i.enCocina > 0)
                    .map((item) => (
                      <div
                        key={`${item.id}-enCocina`}
                        className="flex justify-between items-center p-3 bg-orange-100 rounded-xl mb-2"
                      >
                        <span>{item.name}</span>
                        <span className="font-bold">{item.enCocina}x</span>
                      </div>
                    ))}
                </div>
              )}
              ...
              {/* ✅ Listos */}
              {agrupados.some((i) => i.listos > 0) && (
                <div className="mb-2">
                  <h3 className="text-green-700 font-semibold mb-1">✅ Listos para servir</h3>
                  {agrupados
                    .filter((i) => i.listos > 0)
                    .map((item) => (
                      <div
                        key={`${item.id}-listos`}
                        className="flex justify-between items-center p-3 bg-green-100 rounded-xl mb-2"
                      >
                        <span>{item.name}</span>
                        <div className="flex items-center gap-2">
                          <span className="font-bold">{item.listos}x</span>
                          <button
                            className="bg-primary text-white px-3 py-1 rounded-lg font-bold"
                            onClick={() => {
                              (mesa.items ?? [])
                                .filter((i) => i.id === item.id && i.estado === "listo")
      ...
      {mesaParaCobrar && (
        <ModalCobroEditable mesaId={mesaParaCobrar} setMesaParaCobrar={setMesaParaCobrar} />
      )}
    </main>
  );
}
