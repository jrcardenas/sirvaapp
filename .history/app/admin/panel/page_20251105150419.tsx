        ...
        <div className="space-y-6">
          ...
          {mesasActivas.map(([mesaId, mesa]) => {
            const agrupados = Object.values(
              mesa.items.reduce((acc: Record<string, GrupoItem>, it: PedidoItem) => {
                if (!acc[it.id]) {
                  acc[it.id] = {
                    id: it.id,
                    name: it.name,
...
{/* ✅ Modal Cobro - Total de toda la cuenta */}
{mesaParaCobrar && (
  <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50">
    <div className="bg-white rounded-xl p-6 w-80 text-gray-900 shadow-xl">
      ...
      {(() => {
        ...
        const total = mesa.items.reduce((sum, item) => sum + item.price, 0);

        return (
          <p className="text-center text-3xl font-extrabold mb-6 text-primary">
            {total.toFixed(2)} €
          </p>
        );
      })()}
      ...
      <div className="flex flex-col gap-2">
        <button
          className="bg-primary text-white py-2 rounded-xl font-bold"
          onClick={() => {
        ...
        <button
          className="bg-gray-300 py-2 rounded-xl font-semibold"
          onClick={() => setMesaParaCobrar(null)}
        >
          ❌ Cancelar
        </button>

        <button className="bg-blue-600 text-white py-2 rounded-xl font-bold opacity-60 cursor-not-allowed">
