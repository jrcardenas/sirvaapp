...
export default function AdminPanel() {
  ...
  // ✅ NUEVO — Admin se actualiza cuando cliente llama
  useEffect(() => {
    const ch = new BroadcastChannel("notify_channel");

    ch.onmessage = (event) => {
      const { mesaId, llamada } = event.data;
      if (!mesaId || typeof llamada !== "boolean") return;

      useOrderStore.setState((prev) => ({
        mesas: {
          ...prev.mesas,
          [mesaId]: {
            ...(prev.mesas[mesaId] ?? { items: [], cuentaSolicitada: false }),
            llamada
          }
        }
      }));
