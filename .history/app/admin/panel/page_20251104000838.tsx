...
export default function AdminPanel() {
  ...
  // ✅ Reproducir sonido cuando hay cambios
  useEffect(() => {
    if (!soundReady) return;
    dingRef.current?.play().catch(() => null);
  }, [mesas, soundReady]);

  // ✅ Mesas con pedidos o llamada activa
  const mesasActivas = Object.entries(mesas).filter(
    ([, mesa]) => (mesa.items?.length ?? 0) > 0 || mesa.llamada === true
  );
      ...
      {mesasActivas.length === 0 ? (
        ...
        <div className="space-y-6">
          {mesasActivas.map(([mesaId, mesa]) => {
            const pendientes = mesa.items?.filter((i) => !i.entregado).length ?? 0;

            return (
              <div
                key={mesaId}
                className="bg-surface p-4 rounded-xl shadow-md border border-border"
              >
                {/* ✅ Encabezado Mesa */}
                <div className="flex justify-between items-center mb-3">
                  ...
                  {mesa.llamada && (
                    <span className="bg-primary text-white py-1 px-3 text-sm rounded-full font-semibold animate-pulse">
                      🛎️ Llamando
                    </span>
                  )}

                  {pendientes > 0 && !mesa.llamada && (
                  ...
                  <div className="space-y-2 mb-4">
                    {mesa.items.map((item) => (
                      <div
                        ...
                        className={`flex justify-between items-center p-2 rounded-lg border text-sm transition ${
                          item.entregado
