                  ...
                  <div className="bg-blue-600 text-white p-3 rounded-xl mb-3 text-center font-bold animate-pulse flex justify-between items-center">
                    ...
                    <button
                      onClick={() => {
                        ...
                        // ✅ NUEVO — Aviso al cliente
                        const notify = new BroadcastChannel("notify_channel");
                        notify.postMessage({
                          mesaId,
                          mensaje: "cuentaEnPreparacion"
                        });
                        notify.close();

                        const pedidos = new BroadcastChannel("pedidos_channel");
                        pedidos.postMessage({ mesas: useOrderStore.getState().mesas });
                        pedidos.close();
                      }}
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      <button
                        onClick={() => {
                          ...
                          // ✅ NUEVO — Aviso cliente pedido en camino
                          const notify = new BroadcastChannel("notify_channel");
                          notify.postMessage({
                            mesaId,
                            mensaje: "pedidoEnCamino"
                          });
                          notify.close();
                        }}
            ...
            {(() => {
              ...
              const itemsAgrupados = Object.values(
                mesa.items.reduce((acc, item) => {
                  ...
                  return acc;
                }, {} as Record<string, AgrupadoProducto>)
              );
...
function CrearProductoPersonalizado({ mesaId, nombreInicial, onCancel }: any) {
