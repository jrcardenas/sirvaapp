...
{/* ✅ Pedido en curso */}
{useOrderStore.getState().mesas[mesaId]?.items?.length > 0 && (
  <div className="bg-gray-100 p-4 rounded-xl mb-6">
    <h3 className="font-bold text-lg mb-2 text-gray-900">Tu pedido en curso</h3>

    {useOrderStore.getState().mesas[mesaId].items.map((item) => (
      <div
        key={`${item.id}-${item.timestamp}`}
        className={`flex justify-between items-center text-sm mb-1 p-2 rounded-md ${
          item.entregado
      ...
      >
        ...
        <span>
          {item.entregado ? "✅ Servido" : "⏳ Pendiente"}
        </span>
      </div>
    ))}
  </div>
