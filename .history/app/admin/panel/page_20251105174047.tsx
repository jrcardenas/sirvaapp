...
export default function AdminPanel() {
  ...
  // ✅ Sonido SOLO cuando el cliente: nuevo pedido, llamada o cuenta
  useEffect(() => {
    ...
    pedidos.onmessage = (ev) => {
      ...
      // Sync de estado de mesas
      if (data.mesas) {
        useOrderStore.setState({ mesas: data.mesas });
      }

      // Disparadores válidos de sonido (compatibilidad con formato viejo y nuevo)
      const isTriggerTipo =
        typeof data.tipo === "string" &&
        ["nuevoPedido", "llamada", "cuenta"].includes(data.tipo);

      const isTriggerFlags =
        data.nuevoPedido === true || data.llamada === true || data.cuenta === true;

      if (isTriggerTipo || isTriggerFlags) {
  ...
  const agregarProductoMenu = (mesaId: string, product: MenuItemType) => {
              ...
              <div key={mesaId} className="p-4 rounded-2xl border shadow bg-white">
                ...
                {mesa.llamada && (
                  <div className="bg-red-600 text-white p-3 rounded-xl mb-3 text-center font-bold animate-pulse flex justify-between items-center">
                    ...
                    <button
                      onClick={() => {
                        // Marcar atendida (esto hace _sync → NO suena)
                        atenderLlamada(mesaId);

                        // Avisar al cliente que fue atendido (notify_channel)
                        const notify = new BroadcastChannel("notify_channel");
                        notify.postMessage({ mesaId, respondido: true });
                        notify.close();
                      }}
