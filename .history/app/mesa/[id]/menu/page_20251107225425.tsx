...
notify.onmessage = (ev) => {
  ...
  if (ev.data?.respondido) {
    // Cierra todos los toasts previos
  ...
  // Cierra todos los toasts antes de mostrar uno nuevo
  setToastCall(false);
  setToastPedido(false);
  setToastCuenta(false);
  setToastServido(false);
  ...
  switch (mensaje) {
    // 🍽 Pedido en camino (desde cocina)
    case "pedidoEnCamino":
      setToastPedido(true);
      setTimeout(() => setToastPedido(false), 3000);
      break;

    // 🍽 Pedido servido (final o barra)
    case "pedidoServido":
      // Prioridad máxima → reemplaza cualquier otro toast activo
      setToastServido(true);
      setTimeout(() => setToastServido(false), 3000);
      break;

    // 💳 Cuenta en preparación o en camino
