...
export default function MenuPage() {
  ...
  return (
    <>
      <main className="min-h-screen bg-background p-5 pb-32 max-w-md mx-auto pt-14">
        <h1 className="text-2xl font-bold text-center mb-5 text-textDark">Carta 📋</h1>

        {categorias.map((cat) => (
          <div key={cat} className="mb-6">
            <button
              className="w-full flex justify-between items-center text-lg font-semibold py-2 text-textDark"
                ...
                .map((item) => {
                  ...
                  return (
                    <div key={item.id} className="flex justify-between items-center mb-3">
                      <div>
                        <p className="font-semibold text-textDark">{item.nombre}</p>
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        <button
                          ...
                          className={`w-8 h-8 rounded-full flex items-center justify-center font-bold transition ${
                            qty === 0
                              ? "border-gray-300 text-gray-400 opacity-40 border"
                              : "border-danger text-danger border active:scale-90"
                          }`}
                        >
                          -
                        </button>

                        <span className="w-6 font-bold text-center text-textDark">{qty}</span>

                        {/* ✅ Botón sumar perfecto */}
                        <button
                          className="w-8 h-8 rounded-full bg-primary text-white font-bold flex items-center justify-center active:scale-90"
                          onClick={() =>
                            addItem({
                              id: item.id,
                              name: item.nombre,
        ...
        {total > 0 && (
          <div className="fixed bottom-0 left-0 w-full bg-surface border-t border-gray-200 shadow-xl">
            <div className="max-w-md mx-auto p-4">
              <div className="flex justify-between text-lg font-bold text-textDark mb-2">
                <span>Total</span>
        ...
        <div className="fixed inset-0 bg-black/40 backdrop-blur-sm flex items-center justify-center z-50">
          <div className="bg-white rounded-xl p-5 w-80 shadow-xl">
            ...
            <div className="flex gap-2">
              ...
              <button
                className="w-1/2 py-2 bg-primary text-white rounded-lg font-semibold"
