...
export default function MenuPage() {
  ...
  return (
    <>
      <main className="min-h-screen bg-[var(--color-background)] p-4 pb-32 max-w-md mx-auto pt-14">
                ...
                .map((item) => {
                  ...
                  return (
                    <div key={item.id} className="flex justify-between mb-3">
                      <div>
                        <p className="font-semibold text-[var(--color-text-dark)]">{item.nombre}</p>
                        <p className="text-[var(--color-text-muted)] text-sm">{item.precio.toFixed(2)} €</p>
                      </div>
        ...
        <div className="fixed inset-0 bg-black/40 backdrop-blur-sm flex items-center justify-center z-50">
          <div className="bg-white rounded-xl p-5 w-80 shadow-2xl">
            ...
            <div className="flex gap-2">
              <button className="w-1/2 py-2 bg-gray-300 rounded-lg" onClick={() => setShowConfirm(false)}>
                ❌ Cancelar
              </button>
              <button className="w-1/2 py-2 bg-[var(--color-primary)] text-white rounded-lg font-semibold"
