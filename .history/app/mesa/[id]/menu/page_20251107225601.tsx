...
export default function MenuPage() {
  ...
  const [toastPedido, setToastPedido] = useState(false);
  const [toastCuenta, setToastCuenta] = useState(false);
  const [toastServido, setToastServido] = useState(false); // ✅ nuevo toast para pedido servido
...
notify.onmessage = (() => {
  ...
  return (ev) => {
    ...
    const mensaje = ev.data?.mensaje;
    const now = Date.now();

    // Evita duplicados si llegan mensajes similares en < 1 segundo
    if (mensaje === lastMessage && now - lastTime < 1000) return;
    if (
      lastMessage === "pedidoEnCamino" &&
      mensaje === "pedidoServido" &&
      now - lastTime < 1000
    ) {
      // Reemplaza el toast anterior por el nuevo (más importante)
      setToastPedido(false);
    }
    ...
    // 🔔 Avisos relacionados con pedidos, cuenta y servicio
    // Cierra todos los demás toasts antes de mostrar el nuevo
    setToastCall(false);
    setToastCuenta(false);

    switch (mensaje) {
      case "pedidoEnCamino":
        setToastPedido(true);
        setToastServido(false);
        setTimeout(() => setToastPedido(false), 3000);
        break;

      case "pedidoServido":
        setToastPedido(false);
        setToastServido(true);
        setTimeout(() => setToastServido(false), 3000);
        break;

      case "cuentaEnPreparacion":
      case "cuentaEnCamino":
        setToastCuenta(true);
  ...
  return (
    <>
      ...
      {/* 🔔 TOASTS */}
      {showToast && (
        <Toast color="bg-green-600" text="🍽️ Pedido enviado con éxito" />
      )}
      {toastCall && (
        <Toast color="bg-blue-600" text="👋 El camarero está en camino" />
      )}
      {toastPedido && (
        <Toast color="bg-green-700" text="✅ Tu pedido va en camino" />
      )}
      {toastCuenta && (
        <Toast color="bg-purple-600" text="💳 La cuenta está en camino" />
      )}
      {toastServido && (
        <Toast color="bg-green-600" text="🍽️ Tu pedido está servido" />
      )}
    </>
  );
}

/* ✅ Componente reutilizable de toast */
function Toast({ color, text }: { color: string; text: string }) {
  return (
    <div
      className={`fixed bottom-32 left-1/2 -translate-x-1/2 ${color} text-white px-4 py-2 rounded-lg shadow-md font-bold animate-pulse`}
    >
      {text}
    </div>
  );
