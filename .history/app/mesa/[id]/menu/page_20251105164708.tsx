...
export default function MenuPage() {
  ...
  // ✅ Sync tiempo real cuando camarero marca servido, cobra, atiende cuenta…
  useEffect(() => {
  ...
  return (
    <>
      <main className="min-h-screen bg-white p-5 pb-40 max-w-md mx-auto pt-14">
        ...
        {mesaActual?.items?.length > 0 && (
          <div className="bg-gray-100 p-4 rounded-xl mb-6">
            ...
            {(() => {
              const agrupado = mesaActual.items.reduce((acc, item) => {
                if (!acc[item.id]) {
                  acc[item.id] = {
                    id: item.id,
                    name: item.name,
                ...
                return acc;
              }, {} as Record<string, { id: string; name: string; qty: number; entregados: number }>);

              return Object.values(agrupado).map((item) => {
                const todosServidos = item.entregados === item.qty;
                return (
                  <div
                    key={item.id}
                    className={`flex justify-between items-center text-sm mb-2 px-2 py-1 rounded-md ${
                      todosServidos
                  ...
                  >
                    ...
                    <span>
                      {todosServidos
                        ? "✅ Servido"
                        : `⏳ ${item.qty - item.entregados} Pendiente(s)`}
                    </span>
                  </div>
                );
              });
                  ...
                  return (
                    <div key={item.id} className="flex justify-between items-center mb-3">
                      ...
                      <div className="flex items-center gap-3">
                        ...
                        <button
                          className="w-10 h-10 rounded-full bg-green-600 text-white font-bold flex items-center justify-center shadow active:scale-90"
                          onClick={() =>
                            addItem({
                              id: item.id,
                              name: item.nombre,
          ...
          <div className="fixed bottom-0 left-0 w-full bg-white border-t border-gray-200 shadow-xl">
            <div className="max-w-md mx-auto p-4">
              ...
              <Button
                ...
                className="w-full bg-green-600 text-white font-bold py-3 rounded-2xl shadow-md active:scale-95 transition"
              >
                ✅ Realizar Pedido
              </Button>
            </div>
        ...
        <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50">
          <div className="bg-white rounded-xl p-5 w-80 shadow-xl">
            ...
            <div className="flex gap-2">
              ...
              <button
                className="w-1/2 bg-green-600 text-white font-bold py-3 rounded-2xl shadow-md active:scale-95 transition"
      ...
      {showToast && (
        <div className="fixed bottom-32 left-1/2 -translate-x-1/2 bg-green-600 text-white px-4 py-2 rounded-lg shadow-lg">
