...
export default function MenuPage() {
  ...
  return (
    <>
      <main className="min-h-screen bg-background p-5 pb-32 max-w-md mx-auto pt-14 text-textDark">
                  ...
                  return (
                    <div key={item.id} className="flex justify-between items-center mb-3">
                      <div>
                        ...
                        <p className="text-textMuted text-sm">{item.precio.toFixed(2)} €</p>
                      </div>
                      ...
                      <div className="flex items-center gap-3">
                        <button
                          ...
                          className={`w-10 h-10 rounded-full flex items-center justify-center font-bold transition ${
                            qty === 0
                              ? "bg-disabledBg text-disabledText"
                              : "border-2 border-danger text-danger active:scale-90"
                          }`}
                        >
                          -
                        </button>

                        <span className="w-6 font-bold text-center">{qty}</span>

                        <button
                          className="w-10 h-10 rounded-full bg-primaryLight text-white font-bold flex items-center justify-center shadow active:scale-90 transition"
                          onClick={() =>
                            addItem({
                              id: item.id,
                              name: item.nombre,
          ...
          <div className="fixed bottom-0 left-0 w-full bg-surface border-t border-border shadow-lg">
            <div className="max-w-md mx-auto p-4">
              ...
              <Button
                ...
                className="w-full bg-primaryLight text-white font-bold py-3 rounded-2xl shadow-md active:scale-95 transition"
              >
                ✅ Realizar Pedido
              </Button>
            </div>
      ...
      {showConfirm && (
        <div className="fixed inset-0 bg-black/40 backdrop-blur-sm flex items-center justify-center z-50">
          <div className="bg-surface rounded-xl p-5 w-80 shadow-xl">
            <h3 className="text-lg font-bold text-center mb-4 text-textDark">Confirmar Pedido</h3>

            {items.map((i) => (
              <div key={i.id} className="flex justify-between text-sm mb-2 text-textDark">
                <span>{i.qty}x {i.name}</span>
                <span>{(i.price * i.qty).toFixed(2)} €</span>
              </div>
            ))}

            <p className="text-center font-bold mb-4 text-textDark">Total: {total.toFixed(2)} €</p>

            <div className="flex gap-2">
              <button className="w-1/2 py-2 bg-disabledBg text-textDark rounded-lg font-semibold"
                onClick={() => setShowConfirm(false)}
              >
                ❌ Cancelar
              </button>

              <button className="w-1/2 py-2 bg-primary text-white rounded-lg font-semibold active:scale-95 transition"
      ...
      {showToast && (
        <div className="fixed bottom-32 left-1/2 -translate-x-1/2 bg-primary text-white px-4 py-2 rounded-lg shadow-lg animate-toast">
