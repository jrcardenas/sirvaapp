...
export default function MenuPage() {
  ...
  const [toastPedido, setToastPedido] = useState(false); // ✅ NUEVO
  const [toastCuenta, setToastCuenta] = useState(false); // ✅ NUEVO

  // ✅ ESCUCHA NOTIFICACIONES DEL CAMARERO
  useEffect(() => {
    if (!mesaId) return;
  ...
  const handleConfirmOrder = () => {
    ...
    const pedidos = new BroadcastChannel("pedidos_channel");
    pedidos.postMessage({ tipo: "nuevoPedido", mesaId });
    pedidos.close();
    ...
    <>
      ...
      {toastCuenta && (
        <div className="fixed bottom-32 left-1/2 -translate-x-1/2 bg-purple-600 text-white px-4 py-2 rounded-lg shadow-md font-bold animate-bounce">
          💳 El camarero está preparando la cuenta
        </div>
      )}
    </>
  );
