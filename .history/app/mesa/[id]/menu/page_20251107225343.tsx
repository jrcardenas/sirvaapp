...
notify.onmessage = (ev) => {
  ...
  // Evita toasts duplicados en menos de 2 segundos
  let lastToastTime = 0;
  const now = Date.now();
  ...
  switch (ev.data?.mensaje) {
    // 🍽 Pedido en camino (cuando camarero marca “Servir” en cocina)
    case "pedidoEnCamino":
      setToastPedido(true);
      setTimeout(() => setToastPedido(false), 3000);
      break;

    // 🍽 Pedido servido completamente (barra o finalización)
    case "pedidoServido":
      // Si justo acaba de llegar “pedidoEnCamino”, ignoramos este
      if (toastPedido) return;
