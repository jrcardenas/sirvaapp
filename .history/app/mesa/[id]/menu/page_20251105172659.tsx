...
export default function MenuPage() {
  ...
  const [toastCall, setToastCall] = useState(false); // ✅ Nuevo estado

  // ✅ ESCUCHA LLAMADAS Y RESPUESTA DEL CAMARERO
  useEffect(() => {
    if (!mesaId) return;
  ...
  // ✅ Broadcast para sincronizar pedidos
  useEffect(() => {
        ...
        <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50">
          <div className="bg-white p-4 rounded-xl w-80">
            ...
            <div className="flex gap-2 mt-4">
              <button className="w-1/2 bg-gray-300 rounded-lg py-2" onClick={() => setShowConfirm(false)}>
                ❌ Cancelar
              </button>
              <button className="w-1/2 bg-green-600 text-white font-bold py-2 rounded-lg" onClick={handleConfirmOrder}>
                ✅ Confirmar
              </button>
            </div>
          </div>
