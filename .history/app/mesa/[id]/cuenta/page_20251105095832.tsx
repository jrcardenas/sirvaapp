...
export default function CuentaPage() {
  ...
  const itemsAgrupados = Object.values(
    items.reduce((acc, item) => {
      ...
      return acc;
    }, {} as Record<string, { id: string; name: string; price: number; qty: number }>)
  );
        ...
        {items.length === 0 ? (
          ...
          <div className="space-y-3">
            {itemsAgrupados.map((i) => (
              <div
                key={i.id}
                className="flex justify-between items-center p-3 rounded-lg shadow-sm border bg-white border-gray-300"
              >
