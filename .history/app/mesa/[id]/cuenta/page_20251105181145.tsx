...
export default function CuentaPage() {
  ...
  // ✅ Estado local para bloquear el botón inmediatamente al pedir la cuenta
  const [cuentaSolicitada, setCuentaSolicitada] = useState(false);

  // ✅ Toasts de feedback al cliente
  ...
  // ✅ Sync en tiempo real desde el camarero (estado de mesas)
  useEffect(() => {
  ...
  // ✅ Escucha de notificaciones del camarero (feedback visual)
  useEffect(() => {
    ...
    notify.onmessage = (ev) => {
      ...
      switch (data.mensaje) {
        case "camareroEnCamino":
          showToast("👋 El camarero está en camino");
          break;
        case "pedidoEnCamino":
          showToast("🍽️ Tu pedido va en camino");
          break;
        case "cuentaEnCamino":
          // Asegura que el cliente vea el estado correcto aunque recargue
          setCuentaSolicitada(true);
          showToast("💳 La cuenta está en camino");
          break;
        case "mesaCobrada":
          showToast("✅ Mesa cobrada, ¡gracias!");
          // El estado de la mesa se vacía por el sync de pedidos_channel
          break;
        default:
          break;
      }
  ...
  const handlePedirCuenta = () => {
    ...
    // Aviso al panel del camarero (para ding + estado)
    const pedidos = new BroadcastChannel("pedidos_channel");
    pedidos.postMessage({
      tipo: "cuenta",
      mesaId,
  ...
  return (
    <main className="min-h-screen bg-white p-5 max-w-md mx-auto pt-20 pb-40 text-gray-900">
      ...
      {/* ✅ Zona fija inferior total + pedir cuenta */}
      {items.length > 0 && (
        <div className="fixed bottom-0 left-0 w-full bg-white border-t shadow-xl">
          <div className="max-w-md mx-auto p-4 space-y-2">
            ...
            {/* Mostramos el botón mientras no esté pedida ni marcada en el store */}
