...
export default function CamareroPage() {
  ...
  const mesasOrdenadas = mesasIds.sort((a, b) => {
    ...
    const pa = ma.items.filter((i: any) => !i.entregado).length;
    const pb = mb.items.filter((i: any) => !i.entregado).length;
    ...
    const sa = ma.items.filter((i: any) => i.entregado).length;
    const sb = mb.items.filter((i: any) => i.entregado).length;
    return sb - sa;
  });
    ...
    <main className="min-h-screen bg-background p-5 pt-20 max-w-lg mx-auto">
      ...
      <div className="space-y-4">
        {mesasOrdenadas.map((mesaId) => {
          ...
          const pendientes = mesa.items.filter((i: any) => !i.entregado);
          const servidos = mesa.items.filter((i: any) => i.entregado);
          ...
          const groupedPendientes = Object.values(
            pendientes.reduce<Record<string, ItemAgrupado>>((acc, it) => {
              if (!acc[it.id]) acc[it.id] = { id: it.id, name: it.name, price: it.price, qty: 0 };
              acc[it.id].qty++;
              return acc;
            }, {})
          );
                      ...
                      {mesaEditando === mesaId ? (
                        <div className="flex gap-2">
                          <button className="bg-red-600 text-white px-2 rounded-lg"
                            onClick={() => reducirItem(mesaId, p.id)}>➖</button>
                          <button className="bg-green-600 text-white px-2 rounded-lg"
                            onClick={() => incrementarItem(mesaId, p.id)}>➕</button>
                          <button className="bg-primary text-white px-2 rounded-lg"
                            onClick={() =>
                              pendientes
                                .filter((i: any) => i.id === p.id)
                                .forEach((i: any) => marcarServido(mesaId, i.timestamp))
                            }
                          >
                            ✅
                          </button>
