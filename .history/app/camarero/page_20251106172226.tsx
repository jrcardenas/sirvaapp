      ...
      <div className="space-y-4">
        {mesasOrdenadas.map((mesaId) => {
          ...
          const groupedPendientes = Object.values(
            pendientes.reduce<Record<string, any>>((acc, it) => {
              if (!acc[it.id]) acc[it.id] = { ...it, qty: 0 };
              acc[it.id].qty++;
              return acc;
            }, {})
          );
          ...
          const groupedServidos = Object.values(
            servidos.reduce<Record<string, any>>((acc, it) => {
              if (!acc[it.id]) acc[it.id] = { ...it, qty: 0 };
              acc[it.id].qty++;
              return acc;
            }, {})
