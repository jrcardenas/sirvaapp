...
type GrupoItem = {
  ...
  price: number;
  cantidad: number;
  estado: "pendienteCocina" | "enCocina" | "listo";
};
...
export default function CocinaPage() {
  ...
  useEffect(() => {
    ...
    for (const [mesaId, mesa] of Object.entries(mesas)) {
      ...
      const items = mesa.items.filter((i) =>
        ["pendienteCocina", "enCocina", "listo"].includes(i.estado ?? "")
      );

      const grupos = Object.values(
        items.reduce<Record<string, GrupoItem>>((acc, it) => {
          ...
          if (!acc[key]) {
            acc[key] = {
              ...
              price: it.price,
              cantidad: 0,
              estado: it.estado as "pendienteCocina" | "enCocina" | "listo",
            };
          }
          acc[key].cantidad += 1;
          return acc;
                ...
                <div className="flex gap-2">
                  {item.estado === "pendienteCocina" && (
                    <button
                      ...
                      onClick={() =>
                        actualizarEstadoMesa(mesaId, item.id, "enCocina")
                      }
                    >
                      🍳 Preparar
                    </button>
                  ...
                  {item.estado === "enCocina" && (
                    <button
                      ...
                      onClick={() => actualizarEstadoMesa(mesaId, item.id, "listo")}
                    >
                      ✅ Listo
                    </button>
                  )}
  ...
  function actualizarEstadoMesa(
    mesaId: string,
    itemId: string,
    nuevoEstado: "enCocina" | "listo"
  ) {
    const mesa = mesas[mesaId];
    if (!mesa) return;

    mesa.items
      .filter((i) => i.id === itemId && ["pendienteCocina", "enCocina"].includes(i.estado ?? ""))
