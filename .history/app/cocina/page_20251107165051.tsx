...
type GrupoItem = {
  ...
  price: number;
  cantidad: number;
  estado: "confirmado" | "enCocina" | "listo";
};
...
export default function CocinaPage() {
  ...
  // 🧮 Agrupar pedidos confirmados/en cocina/listos por mesa
  useEffect(() => {
    ...
    for (const [mesaId, mesa] of Object.entries(mesas)) {
      ...
      const itemsConfirmados = mesa.items.filter((i) =>
        ["confirmado", "enCocina", "listo"].includes(i.estado ?? "")
      );

      const grupos = Object.values(
        itemsConfirmados.reduce<Record<string, GrupoItem>>((acc, it) => {
          ...
          if (!acc[key]) {
            acc[key] = {
              ...
              price: it.price,
              cantidad: 0,
              estado: it.estado as "confirmado" | "enCocina" | "listo",
            };
          }
          acc[key].cantidad += 1;
          return acc;
                ...
                <div className="flex gap-2">
                  ...
                  {item.estado === "enCocina" && (
                    <button
                      ...
                      onClick={() =>
                        actualizarEstadoItemMesa(mesaId, item.id, "listo")
                      }
                    >
                      ✅ Listo
                    </button>
  ...
  // 🔧 Cambiar estado de todos los items de ese producto en una mesa
  function actualizarEstadoItemMesa(
    mesaId: string,
    itemId: string,
    nuevoEstado: "confirmado" | "enCocina" | "listo"
  ) {
    ...
    mesa.items
      .filter(
        (i) =>
          i.id === itemId &&
          ["confirmado", "enCocina"].includes(i.estado ?? "")
      )
