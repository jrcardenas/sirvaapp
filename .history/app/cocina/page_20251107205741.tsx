...
type GrupoItem = {
  ...
  price: number;
  cantidad: number;
  estado: "pendiente" | "preparando" | "preparado";
};
...
export default function CocinaPage() {
  ...
  useEffect(() => {
    ...
    for (const [mesaId, mesa] of Object.entries(mesas)) {
      ...
      const itemsRelevantes = mesa.items.filter((i) =>
        ["pendiente", "preparando", "preparado"].includes(i.estado ?? "")
      );

      const grupos = Object.values(
        itemsRelevantes.reduce<Record<string, GrupoItem>>((acc, it) => {
  ...
  // 🔧 Cambiar estado de todos los items del mismo producto
  function actualizarEstadoItemMesa(
    mesaId: string,
    itemId: string,
    nuevoEstado: "pendiente" | "preparando" | "preparado"
  ) {
    const mesa = mesas[mesaId];
    if (!mesa) return;

    mesa.items
      .filter((i) => i.id === itemId && ["pendiente", "preparando"].includes(i.estado ?? ""))
