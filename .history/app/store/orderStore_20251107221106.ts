...
import { MENU_ITEMS } from "@/data/menu"; // ✅ importamos para leer el destino
...
const updateEstadoItem = (
  ...
  items.map((i) =>
    i.timestamp === timestamp ? { ...i, estado } satisfies PedidoItem : i
  );

const markServido = (items: PedidoItem[], timestamp: number): PedidoItem[] =>
  items.map((i) =>
    i.timestamp === timestamp
      ? { ...i, entregado: true } // no cambiamos estado, solo marcamos servido
      : i
  );
    ...
    (set, get) => ({
      ...
      addToMesa: (mesaId, items) => {
        ...
        const nuevos: PedidoItem[] = items.flatMap((i) => {
          const product = MENU_ITEMS.find((m) => m.id === i.id);
          const destino = product?.destino ?? "cocina"; // ✅ por defecto a cocina
      ...
      addCustomProduct: (mesaId, name, price) => {
        ...
        const nuevo: PedidoItem = {
          ...
          timestamp: Date.now(),
          personalizado: true,
          destino: "cocina", // ✅ por defecto cocina
          estado: "pendiente",
        };
      ...
      marcarServido: (mesaId, timestamp) => {
        ...
        set((state) => {
          ...
          return {
            mesas: {
              ...state.mesas,
              [key]: { ...mesa, items: markServido(mesa.items, timestamp) },
            },
          };
        });

        get()._sync({ tipo: "pedidoServido", mesaId: key });

        if (typeof window !== "undefined") {
          const notify = new BroadcastChannel("notify_channel");
          notify.postMessage({ mesaId: key, mensaje: "pedidoServido" });
          notify.close();
        }
      },
    ...
    }),
    {
      ...
      migrate: (persistedRaw: unknown, from: number): OrderStore => {
        if (!persistedRaw || typeof persistedRaw !== "object")
          return { mesas: {} } as OrderStore;

        const prev = persistedRaw as { mesas?: Record<string, Partial<MesaState>> };
        const mesas: MesaOrders = {};
        for (const [k, v] of Object.entries(prev.mesas ?? {})) {
          mesas[k] = {
            items: Array.isArray(v.items) ? (v.items as PedidoItem[]) : [],
            cuentaSolicitada: !!v.cuentaSolicitada,
            llamada: !!v.llamada,
            customProducts: Array.isArray(v.customProducts)
              ? (v.customProducts as { id: string; name: string; price: number }[])
              : [],
          };
        }
        return { mesas } as OrderStore;
