...
type OrderStore = {
  mesas: MesaOrders;
  modoCocina: "manual" | "auto"; // 👈 NUEVO
  setModoCocina: (modo: "manual" | "auto") => void; // 👈 NUEVO
...
export const useOrderStore = create<OrderStore>()(
  persist(
    (set, get) => ({
      mesas: {},
      modoCocina: "manual", // 👈 por defecto manual

      setModoCocina: (modo) => set({ modoCocina: modo }),

      /* 🧾 Cliente hace pedido */
      addToMesa: (mesaId, items) => {
        const state = get().mesas;
        const modo = get().modoCocina; // 👈 usamos el modo actual
        ...
        const nuevos: PedidoItem[] = items.flatMap((i) =>
          Array.from({ length: i.qty }).map(
            (): PedidoItem => ({
              ...
              qty: 1,
              entregado: false,
              // 👇 si modo auto, pasa directo como "confirmado"
              estado: modo === "auto" ? "confirmado" : "pendiente",
      ...
      addCustomProduct: (mesaId, name, price) => {
        const state = get().mesas;
        const modo = get().modoCocina; // 👈 aplicamos también el modo aquí
        const mesa: MesaState = {
          items: [...(state[mesaId]?.items ?? [])],
          cuentaSolicitada: state[mesaId]?.cuentaSolicitada ?? false,
          llamada: state[mesaId]?.llamada ?? false,
      ...
      /* ✅ Confirmar pedido (admin o camarero) */
      confirmarPedido: (mesaId) => {
        const state = get().mesas;
        const mesa = state[mesaId];
        if (!mesa) return;

        const confirmados = mesa.items.map(
          (i): PedidoItem =>
