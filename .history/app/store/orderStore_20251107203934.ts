...
export type PedidoItem = {
  ...
  name: string;
  price: number;
  qty: number;                 // siempre 1 en cada instancia
  entregado: boolean;
...
// Fuente típica que llega desde el “menú” o UI de selección
...
type SyncEvent =
  | { tipo?: "nuevoPedido" | "pedidoConfirmado" | "llamada" | "llamadaAtendida" | "cuenta" | "cuentaAtendida" | "mesaCobrada" | "añadidoPorCamarero" | "actualizadoPorCocina"; mesaId?: string }
  | {};
...
/* ---------- Canal de sincronización (solo cliente) ---------- */
const pedidosChannel =
  typeof window !== "undefined" ? new BroadcastChannel("pedidos_channel") : null;

/* ---------- Tipos de migración (sin any) ---------- */
type PersistedV1Mesa = {
  items?: unknown;
  cuentaSolicitada?: unknown;
  llamada?: unknown;
  customProducts?: unknown;
};
type PersistedV1 = { mesas?: Record<string, PersistedV1Mesa> } | undefined;

type PersistedV2Mesa = {
  items: PedidoItem[];
  cuentaSolicitada: boolean;
  llamada: boolean;
  customProducts: { id: string; name: string; price: number }[];
};
type PersistedV2 = { mesas: Record<string, PersistedV2Mesa> };

const normalizeMesa = (m?: PersistedV1Mesa): MesaState => ({
...
export const useOrderStore = create<OrderStore>()(
  persist(
    ...
    }),
    {
      name: "order-storage",
      version: 3,
      migrate: (persistedRaw: unknown, from: number): OrderStore["mesas"] extends infer _X
        ? { mesas: MesaOrders }
        : never => {
        // v1/v2 -> normaliza; v3+ deja tal cual
        if (!persistedRaw || typeof persistedRaw !== "object") return { mesas: {} };
        if (from >= 3) {
          // ya en formato correcto
          const ok = persistedRaw as PersistedV2;
          return { mesas: ok.mesas ?? {} };
        }
        const prev = persistedRaw as PersistedV1;
