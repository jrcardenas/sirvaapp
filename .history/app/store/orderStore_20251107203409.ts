...
export type PedidoItem = {
  ...
  name: string;
  price: number;
  qty: number;                 // siempre 1 por instancia
  entregado: boolean;
...
/* ---------- Tipos migración sin any ---------- */
type PersistedMesaLoose = {
  items?: unknown;
...
export const useOrderStore = create<OrderStore>()(
  persist(
    ...
    }),
    {
      ...
      // (Opcional) para depurar rehidratado:
      // onRehydrateStorage: () => (state) => {
      //   console.log("rehidratado", state?.mesas);
      // },
    }
  )
);
