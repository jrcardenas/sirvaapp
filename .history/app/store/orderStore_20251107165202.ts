...
export const useOrderStore = create<OrderStore>()(
  persist(
    (set, get) => ({
      ...
      /* 🧾 Cliente hace pedido → directo a cocina */
        ...
        const nuevos: PedidoItem[] = items.flatMap((i) =>
          Array.from({ length: i.qty }).map(
            (): PedidoItem => ({
              ...
              qty: 1,
              entregado: false,
              estado: "confirmado", // 👈 directo a cocina
      ...
      addCustomProduct: (mesaId, name, price) => {
        ...
        const nuevo: PedidoItem = {
          ...
          timestamp: Date.now(),
          personalizado: true,
          estado: "confirmado", // 👈 también confirmado directo
        };
      ...
      llamarCamarero: (mesaId) => {
        ...
        if (!mesa) return;
        set({
          mesas: { ...mesas, [mesaId]: { ...mesa, llamada: true } },
        });
      ...
      atenderLlamada: (mesaId) => {
        ...
        if (!mesa) return;
        set({
          mesas: { ...mesas, [mesaId]: { ...mesa, llamada: false } },
        });
      ...
      pedirCuenta: (mesaId) => {
        ...
        if (!mesa) return;
        set({
          mesas: { ...mesas, [mesaId]: { ...mesa, cuentaSolicitada: true } },
        });
      ...
      atenderCuenta: (mesaId) => {
        ...
        if (!mesa) return;
        set({
          mesas: { ...mesas, [mesaId]: { ...mesa, cuentaSolicitada: false } },
        });
