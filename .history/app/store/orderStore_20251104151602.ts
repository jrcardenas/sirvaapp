...
// ✅ Broadcast channels (comunicación entre pestañas/dispositivos)
...
export const useOrderStore = create<OrderStore>()(
  persist(
    (set, get) => ({
      ...
      // ✅ NUEVO: el camarero marca como atendida la cuenta
      ...
      llamarCamarero: (mesaId) => {
        ...
        set({
          mesas: { ...get().mesas, [mesaId]: { ...mesa, llamada: true } },
        });
      ...
      atenderLlamada: (mesaId) => {
        ...
        set({
          mesas: { ...get().mesas, [mesaId]: { ...mesa, llamada: false } },
        });
...
// ✅ Sync con otros dispositivos (cliente + admin)
if (notifyChannel) {
  notifyChannel.onmessage = (ev) => {
    const { mesaId, llamada, respondido, cuentaSolicitada, cuentaAtendida } = ev.data;
    ...
    if (cuentaAtendida) updatedMesa.cuentaSolicitada = false;
