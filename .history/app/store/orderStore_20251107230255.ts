...
const markServido = (items: PedidoItem[], timestamp: number): PedidoItem[] =>
  items.map((i) =>
    i.timestamp === timestamp
      ? { ...i, entregado: true } // solo marcamos servido
      : i
  );
    ...
    (set, get) => ({
      ...
      addToMesa: (mesaId, items) => {
        ...
        const nuevos: PedidoItem[] = items.flatMap((i) => {
          ...
          const destino = product?.destino ?? "cocina";
      ...
      marcarServido: (mesaId, timestamp) => {
        ...
        // ✅ enviamos aviso al cliente solo si no se ha hecho manualmente
        if (typeof window !== "undefined") {
          const notify = new BroadcastChannel("notify_channel");
          notify.postMessage({ mesaId: key, mensaje: "pedidoServidoAuto" });
          notify.close();
        }
      },
